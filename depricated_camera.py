...
class Camera:
    def __init__(self, min_confidence=0.7, device_index=0):
        self.cap = cv2.VideoCapture(device_index)
        if not self.cap.isOpened():
            raise RuntimeError("❌ Could not access webcam. Check if another app is using it.")
    ...
    def get_frame(self):
        ...
        if results.detections:
            for detection in results.detections:
                ...
                cv2.rectangle(self.display_frame, (x, y), (x + w, y + h),
                              (0, 255, 0), 2)
                cv2.putText(self.display_frame, "Face Detected!",
                            (x, y - 10),
